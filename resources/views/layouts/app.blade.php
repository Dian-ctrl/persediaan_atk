...
<head>
    ...
    <title>Medini Digital – Manajemen ATK</title>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .search-wrap {
            ...
            background: #f5f7f6; border: 1px solid #e0e7e3; border-radius: 8px;
            padding: 7px 14px; font-size: 0.84rem; color: #888;
        }
        .search-wrap i { font-size: 0.9rem; }

        .topbar-right { display: flex; align-items: center; gap: 16px; }
        .topbar-icon-btn {
            width: 36px; height: 36px; border-radius: 8px; border: 1px solid #e5ebe8;
            display: flex; align-items: center; justify-content: center;
            color: #555; font-size: 1rem; cursor: pointer; background: #fff;
        ...
        .user-chip-name { font-size: 0.82rem; font-weight: 600; color: #1a1a1a; line-height: 1.2; }
        .user-chip-role { font-size: 0.65rem; color: #888; }
        .u-avatar {
        ...
        .s-link {
            ...
            color: #4a5a52; text-decoration: none; font-size: 0.84rem;
        ...
        .sidebar-logout {
            ...
            font-size: 0.84rem; width: 100%; border-radius: 8px; transition: all 0.15s;
        }
        ...
        .hero-banner {
            ...
            border-radius: 14px; padding: 28px 32px; color: #fff; position: relative; overflow: hidden;
        }
        .hero-banner::after {
        ...
        .hero-banner h2 { font-size: 1.5rem; font-weight: 800; margin-bottom: 6px; }
        ...
        .si-lime   { background: #f1f8e9; color: #558b2f; }
        ...
        .tbl-md thead th {
            background: #f5f7f6; color: #555; font-weight: 600;
            font-size: 0.74rem; text-transform: uppercase; letter-spacing: 0.5px;
            padding: 11px 14px; border: none; border-bottom: 1px solid #e5ebe8;
        }
        ...
        .status-card {
            ...
            border-radius: 14px; padding: 24px 22px; color: #fff; height: 100%;
        }
        ...
        .status-card h3 { font-size: 1.2rem; font-weight: 800; margin-bottom: 6px; }
...
<div id="topbar">
    <a href="{{ route('dashboard') }}" class="brand-wrap">
        ...
        <div class="brand-text">
            ...
            <small>Balai Desa Medini</small>
        </div>
    </a>
    <div class="topbar-center">
        <div class="search-wrap">
            <i class="bi bi-search"></i>
            <span>Cari aset atau dokumen...</span>
        </div>
    </div>
    <div class="topbar-right">
        ...
        <div class="user-chip">
            <div class="u-avatar">{{ strtoupper(substr(session('admin_name', 'A'), 0, 1)) }}</div>
            <div>
                <div class="user-chip-name">{{ session('admin_name', 'Administrator') }}</div>
...
<div id="sidebar">
    ...
    <a href="{{ route('barang.index') }}" class="s-link {{ request()->routeIs('barang.*') ? 'active' : '' }}">
        <i class="bi bi-boxes"></i> Stok Barang
    </a>

    <div class="nav-section">Transaksi</div>
    <a href="{{ route('transaksi-masuk.index') }}" class="s-link {{ request()->routeIs('transaksi-masuk.*') ? 'active' : '' }}">
        <i class="bi bi-box-arrow-in-down-right"></i> Barang Masuk
    </a>
    <a href="{{ route('transaksi-keluar.index') }}" class="s-link {{ request()->routeIs('transaksi-keluar.*') ? 'active' : '' }}">
        <i class="bi bi-box-arrow-up-right"></i> Barang Keluar
    </a>

    <div class="nav-section">Master Data</div>
    <a href="{{ route('kategori.index') }}" class="s-link {{ request()->routeIs('kategori.*') ? 'active' : '' }}">
        <i class="bi bi-collection-fill"></i> Kategori Barang
    </a>
    <a href="{{ route('satuan.index') }}" class="s-link {{ request()->routeIs('satuan.*') ? 'active' : '' }}">
        <i class="bi bi-tags-fill"></i> Satuan
    </a>

    <div class="nav-section">Laporan</div>
    <a href="{{ route('laporan.index') }}" class="s-link {{ request()->routeIs('laporan.*') ? 'active' : '' }}">
