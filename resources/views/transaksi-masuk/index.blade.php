    ...
    <div class="card-body-hijau">
        <form method="GET" class="row g-2 align-items-end">
            ...
            <div class="col-md-2">
                ...
                <a href="{{ route('transaksi-masuk.index') }}" class="btn btn-secondary btn-sm">Reset</a>
            </div>
        </form>
    </div>
</div>
...
<div class="card-hijau">
    <div class="card-header-hijau">
        <div class="title"><i class="bi bi-box-arrow-in-down-right"></i> Data Barang Masuk ({{ $data->total() }})</div>
        <a href="{{ route('transaksi-masuk.create') }}" class="btn btn-hijau">
        ...
        <table class="table tbl-hijau">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td style="font-size:.8rem">{{ $d->no_dokumen ?? '-' }}</td>
                    <td class="text-center">
                        <a href="{{ route('transaksi-masuk.edit', $d->id) }}" class="btn btn-warning btn-sm">
                            <i class="bi bi-pencil-fill"></i>
                        </a>
                        <form action="{{ route('transaksi-masuk.destroy', $d->id) }}" method="POST" class="d-inline"
                              onsubmit="return confirm('Hapus transaksi ini? Stok barang akan dikurangi kembali.')">
                            @csrf @method('DELETE')
                ...
                @empty
                <tr><td colspan="10" class="text-center py-4 text-muted">Tidak ada data transaksi masuk</td></tr>
                @endforelse
            </tbody>
            @if($data->count() > 0)
            <tfoot>
                <tr style="background:var(--hijau-bg)">
                    <td colspan="7" class="text-end fw-bold" style="color:var(--hijau)">Total Nilai Halaman Ini:</td>
                    <td class="text-end fw-bold" style="color:var(--hijau)">
                        Rp {{ number_format($data->sum(fn($d) => $d->jumlah * $d->harga_satuan),0,',','.') }}
                    </td>
                    <td colspan="2"></td>
                </tr>
            </tfoot>
