...
<div class="card">
    <div class="card-header">
        <div class="card-title">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/></svg>
            Data Barang Keluar ({{ $transaksis->total() }})
        </div>
        ...
        <table class="data-table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><span class="tx-num tx-keluar">-{{ $t->jumlah }} {{ $t->barang->satuan->nama_satuan ?? '' }}</span></td>
                    <td>Rp {{ number_format($t->harga_satuan,0,',','.') }}</td>
                    <td class="total-red">Rp {{ number_format($t->total,0,',','.') }}</td>
                    <td>{{ $t->penerima ?? '-' }}</td>
                    <td>
                        <div style="display:flex;gap:5px">
                            <a href="{{ route('barang-keluar.edit', $t) }}" class="btn btn-warning btn-icon btn-sm">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                            </a>
                            <form method="POST" action="{{ route('barang-keluar.destroy', $t) }}" onsubmit="return confirm('Hapus transaksi ini?')">
                                @csrf @method('DELETE')
                ...
                @empty
                <tr><td colspan="10" style="text-align:center;padding:32px;color:var(--text-muted)">Belum ada transaksi barang keluar</td></tr>
                @endforelse
            </tbody>
        </table>
        @if($transaksis->count() > 0)
        <div style="padding:12px 20px;background:#FEF2F2;border-top:1px solid var(--border);text-align:right;font-weight:700;color:var(--danger)">
...
function doFilter() {
    ...
    let url = '{{ route("barang-keluar.index") }}?';
