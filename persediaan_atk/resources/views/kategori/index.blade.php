            ...
            <table class="data-table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td style="color:var(--text-light);max-width:280px">{{ $kat->keterangan ?? '-' }}</td>
                        <td><span class="stok-num ok">{{ $kat->barang_count }}</span></td>
                        <td>
                            <div style="display:flex;gap:6px">
                                <a href="{{ route('kategori.edit', $kat) }}" class="btn btn-warning btn-icon btn-sm" title="Edit">
        ...
        <div class="card-body">
            <form method="POST" action="{{ route('kategori.store') }}">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="kode" class="form-control" placeholder="Contoh: ATK/01" value="{{ old('kode') }}" required>
                    @error('kode')<div class="form-error">{{ $message }}</div>@enderror
                </div>
                <div class="form-group">
                    <label class="form-label">Nama Kategori <span class="required">*</span></label>
                    <input type="text" name="nama_kategori" class="form-control" placeholder="Nama kategori..." value="{{ old('nama_kategori') }}" required>
                    @error('nama_kategori')<div class="form-error">{{ $message }}</div>@enderror
                </div>
                <div class="form-group">
                    <label class="form-label">Keterangan</label>
                    <textarea name="keterangan" class="form-control" placeholder="Deskripsi singkat kategori...">{{ old('keterangan') }}</textarea>
                </div>
